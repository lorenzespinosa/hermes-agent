...
#### Native stable-Chrome mode on macOS

macOS users whose existing Chrome cookies require the signed stable Chrome
keychain identity can opt into a stricter native lane. It is off by default and
does not replace the packaged-browser behavior above. The lane directly starts
only `/Applications/Google Chrome.app` on an owner-only Hermes snapshot, exposes
CDP only on `127.0.0.1`, and fails closed rather than falling back to a packaged,
cloud, default, CUA, or explicit-CDP browser.

Set it only in the Hermes profile that should use the Work Chrome identity:
...
hermes config set browser.real_profile_source_browser chrome
hermes config set browser.real_profile_source_profile "Profile 1"
hermes config set browser.real_profile_expected_account "you@work.example"
hermes config set browser.real_profile_macos_native true
```

Replace the profile directory and account with the exact Work Chrome values.
Hermes validates both selectors on every launch without logging either value.
Native mode rejects non-macOS hosts, non-stable Chrome, cloud providers,
headless mode, and `browser.cdp_url`, `BROWSER_CDP_URL`, `BU_CDP_URL`, or
`BU_CDP_WS` overrides. It also rejects Camofox and Lightpanda selectors. The
native path exists only behind `browser_exec`; it never enters the built-in
browser, packaged Chrome, cloud-provider, CUA, or legacy snapshot paths.

**Permitted one-time action:** fully quit daily Chrome before the first launch
or any snapshot refresh. Hermes never closes daily Chrome automatically. The
initial acceptance browser is visible so you can reject any unexpected keychain
or permission prompt; Hermes does not click, type into, approve, or retry one.
After a valid snapshot exists, daily Chrome and the native snapshot may run at
the same time because Hermes no longer reads the live profile for that launch.

The owner-only snapshot, lease, and lock all live below
`$HERMES_HOME/browser-supervisor/native-real-profile/` (the snapshot is its
`snapshot/` child). This root is excluded from Hermes backups. Each Chrome
launch gets a persisted generation-bound Browser Use namespace, and the child
process receives only a bounded system environment plus the supervisor's exact
loopback CDP URL. Hermes closes only the PID/start-time-bound native process
after the configured browser inactivity timeout and on clean shutdown. It
retains ambiguous state rather than killing or deleting an owner it cannot
prove.

Changing the backend or native toggle changes the model tool schema. Start a
new conversation after enabling, disabling, or rolling back native mode; Hermes
does not rebuild a long-lived conversation's cached tool prefix mid-session.

**Rollback:** first record the current `browser.backend`,
`browser.use_real_profile`, `browser.real_profile_macos_native`,
`browser.headed`, `browser.inactivity_timeout`, and `browser.command_timeout`.
Then stop every Hermes client for this profile and run:

```bash
hermes browser cleanup-native-profile
```

Require the command to report proven browser, listener, runtime-record, and
snapshot absence before restoring the recorded configuration values (or setting
both consent toggles false). Start a new conversation after restoration. The
consent-off path also refuses reuse immediately and removes only the Hermes
native snapshot after proving that no process owns it. It never changes the live
Chrome profile or Keychain. If cleanup reports ambiguity, leave the snapshot
and runtime record in place, do not restore or force-delete around the proof,
and resolve the exact owner first. Native mode makes no claim about exclusion
from macOS or third-party backups.
